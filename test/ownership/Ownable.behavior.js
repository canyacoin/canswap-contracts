...
function shouldBehaveLikeOwnable(owner, [anyone]) {
  describe('as an ownable', function () {
    ...
    it('changes owner after transfer', async function () {
      (await this.ownable.isOwner({
        from: anyone
      })).should.be.equal(false);
      const {
        logs
      } = await this.ownable.transferOwnership(anyone, {
        from: owner
      });
    ...
    it('should prevent non-owners from transfering', async function () {
      await shouldFail.reverting(this.ownable.transferOwnership(anyone, {
        from: anyone
      }));
    });

    it('should guard ownership against stuck state', async function () {
      await shouldFail.throwing(this.ownable.transferOwnership(null, {
        from: owner
      }));
    });
