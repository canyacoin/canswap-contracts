...
contract CanSwap is Owned {
    ...
    // CreatePool
    function createPool(address _token, string memory _URI, string memory _API, uint256 _amountCAN, uint256 _amountTKN) public returns (bool success) {
    ...
    // Returns all pools
    function pools() public view returns (address[] memory _pools) {
        return arrayTokens;
    } 
    
    // Returns all pools that a staker is staking in
    function pools(address _staker) public view returns (address[] memory _pools) {
        return nArrayPools[_staker];
    } 
    
    // Returns all stakers in a pool
    function pools(address _token) public view returns (address[] memory _stakers) {
        return nArrayStakes[_token];
    } 
    
    // Returns Pool Resources
    function resources(address _token) public view returns (string[] memory _resources) {
    ...
    function _updateMappings(address _from, address _to, uint256 _balX, uint256 _balY, uint256 _Fee) public {
        if(_from == addrCAN){
    ...
    function _updateBalances(address _from, address _to, uint256 _x, uint256 _y, uint256 _fee) public {
        if(_from == addrCAN){
            bal_CAN += _x;
            bal_TKN -= _y;
            fee_TKN += _fee;
    ...
    function sendToken(address _dest, bool _isEther, uint256 _sendValue) public {
    ...
    function _updateThisPool(address _token, string memory _URI, string memory _API) internal {
        
    }
    
    function _stakeInThisPool(address _token, string memory _CAN, string memory _TKN) internal {
    ...
    // Test Method
    function setCreatePool1() public {
    ...
    function _doubleSwap(address _from, address _to, uint256 _x, address _dest) private returns (bool success) {
        ...
        balX = getCANBalance(_from);
        balY = getTKNBalance(_to);
        feeY = getTKNFeeBalance(_to);
        
        y = getOutput(_x, balX, balY);
        liqFeeY = getLiqFee(_x, balX, balY);
